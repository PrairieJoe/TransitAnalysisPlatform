...
통합 브랜치: 로컬 `main`. 세 기능 브랜치를 통합한 뒤 통합 검증을 완료했고, 이번 버전 확정 단계에서 원격 `main`에 반영한다.
...
통합 검증 당시에는 기존 0.5.3 패키지를 사용했다. 최종 0.6.0 패키지와 실제 여수시 데이터 검증 결과는 [`여수시 실데이터 검증 보고서`](2026-09-18-yeosu-real-data-validation.md)에서 관리한다.
