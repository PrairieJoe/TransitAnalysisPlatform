...
const root=resolve('.'); const output=resolve(process.env.ROAD_SHAPES_OUTPUT??join(root,'test-artifacts','road-shapes'));
...
const pbf=resolve(process.env.ROAD_SHAPES_PBF??join(root,'data/osm/south-korea-latest.osm.pbf')); const config=join(output,'config.yml');
