      ...
      <div className="report-layout">
        ...
        <section className="report-area" ref={reportRef}>
          ...
          {isQuality ? <>
            ...
            <div className="route-report-grid"><div className="station-map-card">{routeMapMode === '3d' ? <RouteCongestion3DView metrics={activeRouteMetrics} routeLabel={activeRouteId ?? 'route'} selectedSegmentKey={selectedRouteSegmentKey} onSelectSegment={selectRouteSegment} onFallback={() => setRouteMapMode('2d')} /> : <RouteCongestionMap metrics={activeRouteMetrics} selectedSegmentKey={selectedRouteSegmentKey} onSelectSegment={selectRouteSegment} />}</div><div className="table-card station-table-card"><div className="station-table-heading"><strong>정류장별 차내재차인원·누적 산출</strong><span>이전 재차인원 + 승차 - 하차 = 현재 재차인원입니다.</span></div><RouteCongestionTable metrics={activeRouteMetrics} selectedSegmentKey={selectedRouteSegmentKey} onSelectSegment={selectRouteSegment} /></div></div>
