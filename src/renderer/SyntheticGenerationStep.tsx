...
export default function SyntheticGenerationStep({
  ...
  const effectiveExplanation = explanationCopy ?? {
    before: `현행 노선별 정류장정보(${activeRouteLabel})를 바탕으로 만든 기준 운행계획입니다.`,
  ...
  return <section className="synthetic-step-panel synthetic-generation-step">
    ...
    <div className="synthetic-scenario-route-note" role="note"><strong>정류장 구성은 이전 단계에서 설정했습니다.</strong><span>현행 {baseStopIds.length}개 · 개편안 {scenarioStopIds.length}개 · 목록의 변경 상태와 순서를 확인한 뒤 생성하세요.</span><span>생성 후 정류장 변경 요약과 개편안 GTFS 결과를 확인할 수 있습니다.</span></div>
    ...
    <div className="synthetic-explanation-card" role="note"><div className="synthetic-explanation-heading"><strong>현행 GTFS와 개편안 GTFS를 이렇게 비교합니다</strong><span>현행 노선과 사용자가 만든 개편안을 같은 조건으로 비교합니다.</span></div><div className="synthetic-explanation-grid"><div><small>현행 GTFS</small><strong>기준 운행계획</strong><span>{effectiveExplanation.before}</span></div><div><small>개편안 GTFS</small><strong>개편 운행계획</strong><span>{effectiveExplanation.after}</span></div></div></div>
    <button className="primary-button full" onClick={onGenerate}>현행·개편안 GTFS 생성 <span>→</span></button>
    ...
    {!result ? <div className="synthetic-result-empty"><span className="empty-icon">↗</span><strong>아직 생성된 결과가 없습니다</strong><span>핵심 입력을 확인하고 생성 버튼을 눌러 주세요.</span></div> : <>
      ...
      {scenarioDelta && <div className="synthetic-scenario-delta"><strong>정류장 변경 요약 · {scenarioDelta.label}</strong><span>추가: {scenarioDelta.addedStopIds.join(', ') || '없음'}</span><span>제거: {scenarioDelta.removedStopIds.join(', ') || '없음'}</span></div>}
